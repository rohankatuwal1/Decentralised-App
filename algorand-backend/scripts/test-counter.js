...
(async () => {
  try {
    ...
    // sign, send, await
    let signedTxn = txn.signTxn(myaccount.sk); console.log("Signed transaction with txID: %s", txId);
...
transactionResponse["txn"]["txn"]["apid"]);
    ...
    // sign, send, await
    let signedTxn2 = txn2.signTxn(myaccount.sk); console.log("Signed transaction with txID: %s", txId2);
